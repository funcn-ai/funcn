...
from packages.sygaldry_registry.components.tools.pdf_search.tool import (
    PDFSearchArgs,
    PDFSearchResponse,
    PDFSearchResult,
    search_pdf_content,
...
class TestPDFSearchTool(BaseToolTest):
    ...
    component_path = Path("packages/sygaldry_registry/components/tools/pdf_search")
    ...
    async def test_basic_pdf_search(self, tmp_path):
        ...
        with patch("packages.sygaldry_registry.components.tools.pdf_search.tool.PyPDF2.PdfReader") as mock_reader:
            # Mock PDF reader
            mock_page = MagicMock()
