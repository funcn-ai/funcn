...
from packages.funcn_registry.components.tools.sqlalchemy_db.tool import (
    AgentState,
    Base,
    DatabaseInfo,
    SQLAlchemyConfig,
...
class TestSQLAlchemyDB(BaseToolTest):
    ...
    async def test_create_tables(self):
        ...
        with patch("packages.funcn_registry.components.tools.sqlalchemy_db.tool.get_sync_engine") as mock_get_engine:
            mock_engine = Mock()
            mock_get_engine.return_value = mock_engine
            
            with patch("packages.funcn_registry.components.tools.sqlalchemy_db.tool.Base.metadata.create_all") as mock_create_all:
    ...
    async def test_store_agent_state_new(self):
        ...
        with patch("packages.funcn_registry.components.tools.sqlalchemy_db.tool.get_async_session") as mock_get_session:
    ...
    async def test_connection_pooling(self):
        ...
        with patch("packages.funcn_registry.components.tools.sqlalchemy_db.tool.create_async_engine") as mock_create_engine:
            with patch("packages.funcn_registry.components.tools.sqlalchemy_db.tool.async_sessionmaker") as mock_sessionmaker:
                # Clear cache to force engine creation
                from packages.funcn_registry.components.tools.sqlalchemy_db.tool import _async_session_cache, _engine_cache
                _engine_cache.clear()
    ...
    async def test_async_to_sync_url_conversion(self):
        ...
        for sync_url, expected_async_url in conversions:
            ...
            with patch("packages.funcn_registry.components.tools.sqlalchemy_db.tool.create_async_engine") as mock_create_engine:
                ...
                with patch("packages.funcn_registry.components.tools.sqlalchemy_db.tool.async_sessionmaker"):
                    async with get_async_session(config) as session:
                        pass
    ...
    async def test_connection_pool_exhaustion(self):
        ...
        with patch("packages.funcn_registry.components.tools.sqlalchemy_db.tool.get_async_session", return_value=async_session_mock):
            result = await store_agent_state(
                config=config,
                agent_id="test_agent",
                key="test_key",
