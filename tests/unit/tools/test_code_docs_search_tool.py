...
class TestCodeDocsSearchTool(BaseToolTest):
    ...
    def get_test_inputs(self):
        ...
        return [
            {"path": "/path/to/project", "query": "authentication", "doc_types": ["docstring", "comment"], "language": "python"},
            {"path": "/path/to/docs", "query": "API usage", "include_readme": True, "include_comments": False},
            {"path": "/path/to/codebase", "query": "configuration", "doc_types": ["readme", "docstring"]},
        ]
...
'''
        ...
        with patch("builtins.open", mock_open(read_data=js_code)):
            with patch("os.walk") as mock_walk:
                ...
                results = tool(tmp_path, "authentication", language="javascript", doc_types=["jsdoc", "comment"])
...
'''
        ...
        with patch("builtins.open", mock_open(read_data=readme_content)):
            with patch("os.walk") as mock_walk:
                mock_walk.return_value = [(str(tmp_path), [], ["README.md", "README.rst", "readme.txt"])]
    ...
    def test_language_filtering(self, tmp_path):
        ...
        with patch("os.walk") as mock_walk:
            ...
            with patch("builtins.open", mock_open()) as mock_file:
                mock_file.side_effect = [mock_open(read_data=content).return_value for content in files.values()]
...
'''
    ...
    def test_exclude_types(self, tmp_path):
        ...
        results = tool(tmp_path, "test", include_comments=False, include_docstrings=True, include_readme=True)
