"""Unit tests for sygaldry add command."""
...
class TestAddCommand(BaseCommandTest):
    """Test cases for the sygaldry add command."""
    ...
    def test_add_component_by_name_success(self, tmp_path):
        ...
        with patch("sygaldry_cli.commands.add.ConfigManager", return_value=mock_config):
            with patch("sygaldry_cli.commands.add.ComponentManager", return_value=mock_component_manager):
                ...
                with patch("rich.prompt.Confirm.ask") as mock_confirm:
                    ...
                    import typer
                    from sygaldry_cli.commands import add as add_module
    ...
    def test_add_component_with_url(self, tmp_path):
        ...
        manifest_url = "https://registry.sygaldry.ai/components/test-agent/manifest.json"
    ...
    def test_add_component_not_found(self, tmp_path):
        ...
        with patch("sygaldry_cli.commands.add.ConfigManager", return_value=mock_config):
            with patch("sygaldry_cli.commands.add.ComponentManager", return_value=mock_component_manager):
                with patch("sygaldry_cli.commands.add.console") as mock_console:
                    with patch("rich.prompt.Confirm.ask") as mock_confirm:
                        mock_confirm.side_effect = [False, False]
    ...
    def test_add_missing_config_file(self, tmp_path):
        """Test adding component when sygaldry.json is missing."""
        # ConfigManager should raise an error when config is missing
        with patch("sygaldry_cli.commands.add.ConfigManager") as mock_config_cls:
            mock_config_cls.side_effect = Exception("No sygaldry.json found")
            ...
            assert "No sygaldry.json found" in str(exc_info.value)
