"""Unit tests for sygaldry init command."""
...
class TestInitCommand(BaseCommandTest):
    """Test cases for the sygaldry init command."""
    ...
    def get_default_schema_data(self):
        ...
        return {
            ...
            "responseModelDirectory": "packages/sygaldry_registry/components/response_models",
    ...
    def run_init_command(self, args, cwd, add_components=False):
        ...
        with patch("sygaldry_cli.commands.init_cmd.load_schema_defaults") as mock_load:
            with patch("sygaldry_cli.commands.init_cmd.Confirm.ask") as mock_confirm:
    ...
    def test_init_with_defaults_creates_config(self, tmp_path):
        """Test that init with --defaults creates sygaldry.json."""
        project_dir = self.create_test_project(tmp_path, with_sygaldry_json=False)

        result = self.run_init_command(["--defaults"], project_dir)

        self.assert_command_success(result)
        self.assert_file_exists(project_dir / "sygaldry.json")

        # Verify config content
        config_data = json.loads((project_dir / "sygaldry.json").read_text())
    ...
    def test_init_creates_directory_structure(self, tmp_path):
        ...
        assert (project_dir / "src" / "sygaldry" / "agents").exists()
        assert (project_dir / "src" / "sygaldry" / "evals").exists()
        assert (project_dir / "src" / "sygaldry" / "prompt_templates").exists()
        assert (project_dir / "src" / "sygaldry" / "tools").exists()
        assert (project_dir / "packages" / "sygaldry_registry" / "components" / "response_models").exists()

    def test_init_with_existing_config_no_force(self, tmp_path):
        """Test that init fails when config exists without --force."""
        project_dir = self.create_test_project(tmp_path, with_sygaldry_json=True)

        result = self.run_init_command(["--defaults"], project_dir)

        # Should show message about existing config
        assert "sygaldry already initialised" in result.output
        assert (project_dir / "sygaldry.json").exists()
    ...
    def test_init_with_force_overwrites_config(self, tmp_path):
        ...
        original_config = json.loads((project_dir / "sygaldry.json").read_text())
        ...
        new_config = json.loads((project_dir / "sygaldry.json").read_text())
    ...
    def test_init_interactive_mode(self, tmp_path):
        ...
        with patch("sygaldry_cli.commands.init_cmd.load_schema_defaults") as mock_load:
            with patch("sygaldry_cli.commands.init_cmd.Prompt.ask") as mock_prompt:
                with patch("sygaldry_cli.commands.init_cmd.Confirm.ask") as mock_confirm:
                    mock_load.return_value = self.get_default_schema_data()
                    ...
                    config = json.loads((project_dir / "sygaldry.json").read_text())
    ...
    def test_init_silent_mode(self, tmp_path):
        ...
        assert len(result.output.strip()) == 0 or "sygaldry initialised" not in result.output
    ...
    def test_init_updates_gitignore(self, tmp_path):
        ...
        assert ".sygaldry/" in gitignore_content
    ...
    def test_init_detects_src_directory(self, tmp_path):
        ...
        # Should use src/sygaldry as base
        config = json.loads((project_dir / "sygaldry.json").read_text())
        assert "src/sygaldry" in config["agentDirectory"]
    ...
    def test_init_detects_app_directory(self, tmp_path):
        ...
        # Should use app/sygaldry as base
        config = json.loads((project_dir / "sygaldry.json").read_text())
        assert "app/sygaldry" in config["agentDirectory"]
            ...
            with patch("sygaldry_cli.commands.init_cmd.Confirm.ask") as mock_confirm:
                with patch("builtins.open", side_effect=OSError("Permission denied")) as mock_open:
                    ...
                    def selective_open(path, mode='r', *args, **kwargs):
                        if 'w' in mode and "sygaldry.json" in str(path):
            ...
            with patch("sygaldry_cli.commands.init_cmd.Prompt.ask") as mock_prompt:
                with patch("sygaldry_cli.commands.init_cmd.Confirm.ask") as mock_confirm:
                    ...
                    def patched_truediv(self, other):
                        ...
                        if str(result).endswith("known_llms.json") and "sygaldry_cli" in str(result):
