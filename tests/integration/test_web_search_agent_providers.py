...
from packages.funcn_registry.components.agents.web_search.agent import (
    SearchProvider,
    WebSearchResponse,
    web_search_agent,
)
...
class TestWebSearchAgentProviders:
    ...
    async def test_multi_provider_fallback_on_failure(self):
        ...
        with patch("packages.funcn_registry.components.tools.duckduckgo_search.tool.DDGS") as mock_ddgs, \
             patch("packages.funcn_registry.components.tools.qwant_search.tool.httpx.AsyncClient") as mock_qwant, \
             patch("packages.funcn_registry.components.agents.web_search.agent.duckduckgo_search", side_effect=Exception("Rate limit exceeded")), \
             patch("packages.funcn_registry.components.agents.web_search.agent.qwant_search") as mock_qwant_search:
    ...
    async def test_parallel_multi_provider_search(self):
        ...
        with patch("packages.funcn_registry.components.agents.web_search.agent.duckduckgo_search") as mock_ddg, \
             patch("packages.funcn_registry.components.agents.web_search.agent.qwant_search") as mock_qwant, \
             patch("packages.funcn_registry.components.agents.web_search.agent.exa_search") as mock_exa:
    ...
    async def test_specialized_provider_selection(self):
        ...
        for test_case in test_cases:
            with patch(f"packages.funcn_registry.components.agents.web_search.agent.{test_case['expected_tool']}") as mock_tool:
    ...
    async def test_provider_specific_features(self):
        ...
        with patch("packages.funcn_registry.components.agents.web_search.agent.exa_search") as mock_exa, \
             patch("packages.funcn_registry.components.agents.web_search.agent.exa_answer") as mock_exa_answer:
    ...
    async def test_privacy_mode_search(self):
        ...
        with patch("packages.funcn_registry.components.agents.web_search.agent.qwant_search") as mock_qwant:
            mock_qwant.return_value = {
                "results": [
                    {
                        "title": "Privacy Guide",
    ...
    async def test_rate_limit_handling_across_providers(self):
        ...
        with patch("packages.funcn_registry.components.agents.web_search.agent.duckduckgo_search", side_effect=mock_ddg_search), \
             patch("packages.funcn_registry.components.agents.web_search.agent.qwant_search", side_effect=mock_qwant_search):
