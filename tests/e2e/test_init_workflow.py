"""End-to-end tests for sygaldry init command workflow."""
...
class TestInitWorkflow(BaseE2ETest):
    ...
    def test_init_creates_complete_project_structure(self, cli_runner, test_project_dir):
        """Test that sygaldry init creates all necessary files and directories."""
        # Run sygaldry init with --yes to use all defaults
        ...
        # Verify sygaldry.json was created
        self.assert_file_exists(test_project_dir / "sygaldry.json")

        # Verify sygaldry.json has correct structure
        sygaldry_json_path = test_project_dir / "sygaldry.json"
        with open(sygaldry_json_path) as f:
            config = json.load(f)

        assert config["$schema"] == "./sygaldry.schema.json"
        ...
        # Check that sygaldry added its entry
        self.assert_file_contains(gitignore_path, ".sygaldry/")

    def test_init_with_custom_directories(self, cli_runner, test_project_dir):
        """Test sygaldry init with custom directory structure."""
        # Run sygaldry init with custom directories
        ...
        # Verify sygaldry.json has custom paths
        with open(test_project_dir / "sygaldry.json") as f:
            config = json.load(f)
    ...
    def test_init_creates_schema_file(self, cli_runner, test_project_dir):
        """Test that sygaldry init creates or uses the schema file."""
        result = self.run_command(cli_runner, ["init", "--yes"], input="n\n")

        self.assert_command_success(result)

        # Check that it warned about missing schema
        assert "sygaldry.schema.json not found" in result.output

    def test_init_in_existing_project(self, cli_runner, test_project_dir):
        """Test sygaldry init behavior in a directory with existing sygaldry.json."""
        # Create an existing sygaldry.json
        existing_config = {
        ...
        sygaldry_json_path = test_project_dir / "sygaldry.json"
        with open(sygaldry_json_path, "w") as f:
            json.dump(existing_config, f, indent=2)

        # Run sygaldry init again
        result = self.run_command(cli_runner, ["init"], input="\n\n\n\nno\n")

        # Should warn about existing configuration (note: British spelling used)
        assert "already initialised" in result.output or "sygaldry.json exists" in result.output

    def test_init_handles_permission_errors(self, cli_runner, test_project_dir, monkeypatch):
        """Test sygaldry init handles directory creation permission errors gracefully."""
