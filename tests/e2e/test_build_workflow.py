"""End-to-end tests for sygaldry build command workflow."""
...
class TestBuildWorkflow(BaseE2ETest):
    ...
    def registry_structure(self, tmp_path):
        ...
        registry_index_path = tmp_path / "packages" / "sygaldry_registry" / "index.json"
    ...
    def test_build_validates_manifests(self, cli_runner, registry_structure):
        ...
        index_path = registry_structure / "packages" / "sygaldry_registry" / "index.json"
    ...
    def test_build_handles_dependencies(self, cli_runner, registry_structure):
        ...
        try:
            ...
            result = self.run_command(cli_runner, ["build", "packages/sygaldry_registry/index.json"])
            self.assert_command_success(result)

            with open(registry_structure / "packages" / "sygaldry_registry" / "index.json") as f:
                index_data = json.load(f)
    ...
    def test_build_empty_registry(self, cli_runner, tmp_path):
        ...
        index_path = tmp_path / "packages" / "sygaldry_registry" / "index.json"
        ...
        try:
            ...
            with open(tmp_path / "packages" / "sygaldry_registry" / "index.json") as f:
                index_data = json.load(f)
    ...
    def test_build_generates_urls(self, cli_runner, registry_structure):
        ...
        try:
            ...
            result = self.run_command(
                cli_runner,
                ["build", "--base-url", "https://registry.sygaldry.ai", "packages/sygaldry_registry/index.json"]
            )
            ...
            assert test_agent["url"] == "https://registry.sygaldry.ai/test_agent.json"

            # Check another component to ensure all have URLs
            test_tool = next(c for c in index_data["components"] if c["name"] == "test_tool")
            assert test_tool["url"] == "https://registry.sygaldry.ai/test_tool.json"
    ...
    def test_build_verbose_output(self, cli_runner, registry_structure):
        ...
        try:
            ...
            result = self.run_command(cli_runner, ["build", "--verbose", "packages/sygaldry_registry/index.json"])
