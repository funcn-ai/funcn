...
def types():
    ...
    console.print("  • Generate docs for all agents: [cyan]funcn docs generate --type agent[/cyan]")
...
def _find_component_path(component_name: str) -> Path | None:
    ...
    # Search in packages/funcn_registry/components/
    registry_path = Path("packages/funcn_registry/components")
...
def _generate_global_editor_rules(components: list[tuple[Path, dict]], editor: str, output_dir: str | None):
    ...
    from funcn_cli.templates.editor_rules import generate_editor_rules
